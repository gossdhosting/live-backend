...
export async function previewUpgrade(req, res) {
  try {
    ...
    // Get user's available account credit
    const accountCredit = await Credit.getBalance(userId);
    const creditToApply = Math.min(accountCredit, proratedAmount);
    const finalDueNow = Math.max(0, proratedAmount - creditToApply);
    ...
    res.json({
      ...
      proration: {
        ...
        creditApplied: creditToApply,
        amountBeforeCredit: proratedAmount,
      },
