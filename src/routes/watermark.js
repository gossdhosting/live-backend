...
// Upload watermark (requires custom_watermark plan feature)
router.post('/upload/:channelId', checkPlanLimit('watermark'), upload.single('watermark'), async (req, res) => {
  try {
...
// Update watermark settings (requires custom_watermark plan feature)
router.put('/:channelId', checkPlanLimit('watermark'), async (req, res) => {
  try {
    const { channelId } = req.params;
