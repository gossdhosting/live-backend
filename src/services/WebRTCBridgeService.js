...
class WebRTCBridgeService {
  ...
  async createPeerConnection(channelId, streamKey) {
    try {
      ...
      // Store input type for use in callbacks where channel object isn't available
      this.channelInputTypes.set(channelId, channel.input_type);
      console.log(`[WebRTC Bridge] Stored input type for channel ${channelId}: ${channel.input_type}`);
        ...
        videoSink.onframe = async ({ frame }) => {
          ...
          try {
            ...
            if (isProcessRunning && frameResolution) {
              ...
              console.log(`[Platform Streaming Check] channel ${channelId}: platformAlreadyStarted = ${platformAlreadyStarted}, processRunning = ${isProcessRunning}, frameResolution = ${frameResolution}`);
  ...
  async startFFmpegBridge(channelId, streamKey, firstFrame) {
    ...
    try {
      ...
      ffmpegArgs.push(
        ...
        '-bufsize', `${bitrate * 2}k`,  // 2x bitrate buffer - allows faster startup while maintaining stability
      ...
      // NOTE: Platform streaming is triggered from handleVideoTrack() first frame handler (line 386-402)
      // with proper 15-second delay. DO NOT trigger it here to avoid race conditions.
      // The first frame handler ensures the stream is truly ready before starting platform streaming.
